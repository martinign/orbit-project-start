...
export const TimelineView: React.FC<TimelineViewProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const createdDates = tasks.map(t => t.created_at ? new Date(t.created_at) : null).filter((d): d is Date => Boolean(d));
      const earliest = createdDates.length ? new Date(Math.min(...createdDates.map(d => d.getTime()))) : today;
      ...
      setMonths(Object.entries(cnts).map(([month, cnt]) => ({
        month,
        days: cnt
      })));
    } catch (err) {
  ...
  const today = new Date();
  return <div className="border rounded-md h-full overflow-hidden">
                  ...
                  <div className="relative w-full">
                    ...
                    <div className="sticky top-0 bg-background z-10">
                      <div className="flex h-8 border-b">
                        {months.map((m, i) => <div key={i} className="flex items-center justify-center border-r text-xs font-medium" style={{
                      width: `${m.days * dayPct}%`
                    }}>
                            {m.month}
                          </div>)}
                    ...
                    const offsetDays = Math.max(0, Math.floor((start.getTime() - days[0].getTime()) / (1000 * 60 * 60 * 24)));
                    const rawDur = Math.ceil(((completed ? end : today).getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
                    const duration = Math.max(1, rawDur);
                    return <div key={task.id} className="h-[33px] relative">
                            <TimelineTaskBar task={task} style={{
                        left: `${offsetDays * dayPct}%`,
                        width: `${duration * dayPct}%`
                      }} onClick={() => setSelectedTask(task)} durationDays={duration} isCompleted={completed} />
                          </div>;
                  })}
    
                      {/* Today indicator */}
                      <div className="absolute top-0 bottom-0 w-[2px] bg-blue-500 z-20" style={{
                    left: `${days.findIndex(d => isToday(d)) * dayPct}%`
                  }} />
                    </div>
                  </div>
                </div>
      ...
      <TaskDetailsDialog task={selectedTask} open={!!selectedTask} onOpenChange={() => setSelectedTask(null)} />
    </div>;
};