// src/components/tasks/timeline/TimelineTaskBar.tsx
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Card
            className={`absolute top-1/2 -translate-y-1/2 h-5 rounded-sm cursor-pointer transition-colors border ${getStatusColor()}`}
            style={style}
            onClick={onClick}
          >
            {durationDays > 3 && (
              <div className="px-2 text-xs font-medium truncate flex items-center h-full">
                ...
                {durationDays > 5 && (
                  <span
                    className={`ml-1 rounded-full text-[10px] py-0 px-1 ${getStatusBadge()}`}
                  >
