...
export const TimelineTaskBar: React.FC<TimelineTaskBarProps> = ({
  ...
  const endDate = task.status === 'completed' && task.updated_at
    ...
    : task.status !== 'completed' ? new Date() : startDate; // If completed and no updated_at, consider duration 0
  ...
  // Calculate the effective end date for duration calculation
  const effectiveEndDate = task.status === 'completed' && task.updated_at
    ? parseISO(task.updated_at)
    : endDate;
