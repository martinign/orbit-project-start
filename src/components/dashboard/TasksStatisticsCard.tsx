...
export function TasksStatisticsCard({
  ...
  const {
    ...
    queryFn: async () => {
      const statuses = filters.status ? [filters.status] : ["not started", "in progress", "pending", "completed", "stucked"];
      const queryResults = [];
      for (const status of statuses) {
        // Start with a base query for each status
        let query = supabase.from("project_tasks").select("id, project:project_id(project_type)").eq("status", status);
      ...
      if (filters.projectType && filters.projectType !== "all") {
        filteredResults = filteredResults.map(resultData => resultData.filter(task => task.project?.project_type === filters.projectType));
      }
  ...
  return <Card>
      ...
      <CardContent className="py-[21px]">
        {isLoading ? <div className="space-y-2">
            ...
            <TaskStatusPieChart data={data || []} onSliceClick={handleSliceClick} />
          </div>}
      </CardContent>
    </Card>;
}