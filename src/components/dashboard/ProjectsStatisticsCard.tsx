...
export function ProjectsStatisticsCard({
  ...
  const {
    ...
    queryFn: async () => {
      let activeQuery = supabase.from("projects").select("status").eq("status", "active");
      let pendingQuery = supabase.from("projects").select("status").eq("status", "pending");
      let completedQuery = supabase.from("projects").select("status").eq("status", "completed");
      let cancelledQuery = supabase.from("projects").select("status").eq("status", "cancelled");
      ...
      const [activeResult, pendingResult, completedResult, cancelledResult] = await Promise.all([activeQuery, pendingQuery, completedQuery, cancelledQuery]);
      if (activeResult.error || pendingResult.error || completedResult.error || cancelledResult.error) {
        throw new Error("Failed to fetch project statistics");
      }
      return [{
        name: "Active",
        value: activeResult.data?.length || 0,
        color: "#4ade80",
        status: "active"
      }, {
        name: "Pending",
        value: pendingResult.data?.length || 0,
        color: "#fbbf24",
        status: "pending"
      }, {
        name: "Completed",
        value: completedResult.data?.length || 0,
        color: "#60a5fa",
        status: "completed"
      }, {
        name: "Cancelled",
        value: cancelledResult.data?.length || 0,
        color: "#f87171",
        status: "cancelled"
      }];
    },
  ...
  return <Card>
      ...
      <CardContent className="py-[32px]">
        {isLoading ? <div className="space-y-2">
          ...
          </div> : total === 0 ? <div className="flex items-center justify-center h-[200px] text-muted-foreground text-sm">
            No projects data available
          </div> : <div className="space-y-4">
            <ProjectsBarChart data={projects || []} onBarClick={handleBarClick} />
