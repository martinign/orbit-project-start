...
const ProjectCard = ({
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest('[data-dropdown-trigger]') || (e.target as HTMLElement).closest('[data-dropdown-content]')) {
      return;
    }
  ...
  return <div className="relative">
      <HoverCard>
        <HoverCardTrigger asChild>
          <Card className="h-[180px] w-full cursor-pointer hover:border-blue-300 transition-all duration-200" onClick={handleCardClick}>
            <CardContent className="p-6 flex flex-col h-full justify-between">
              <div className="absolute top-3 right-3">
                <DropdownMenu>
                  <DropdownMenuTrigger className="h-8 w-8 flex items-center justify-center rounded-full hover:bg-gray-100" data-dropdown-trigger="true" onClick={e => e.stopPropagation()}>
                    <MoreHorizontal className="h-5 w-5 text-gray-500" />
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" data-dropdown-content="true" onClick={e => e.stopPropagation()}>
      ...
      <ProjectDialog open={isEditDialogOpen} onClose={() => setIsEditDialogOpen(false)} onSuccess={handleEditSuccess} project={project} />
    </div>;
};
