...
export const SiteInitiationCSVUploader: React.FC<SiteInitiationCSVUploaderProps> = ({
  ...
  const processCRACSVDataInternal = async (records: CRAData[]): Promise<CRAOperationsResult> => {
    ...
    if (!projectId || !records.length) {
      console.error('Missing project ID or records', {
        projectId,
  ...
  return <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" className="flex items-center gap-2" onClick={handleDownloadTemplate}>
        ...
        <Tabs value={importType} onValueChange={value => {
      ...
      <CardContent>
        {!file ? <div {...getRootProps()} className={`border-2 border-dashed rounded-lg p-10 text-center cursor-pointer transition-colors ${isDragActive ? 'border-primary bg-primary/5' : 'border-muted-foreground/30'} ${isDragReject ? 'border-red-500 bg-red-50' : ''}`}>
            <input {...getInputProps()} />
            <FileUp className="h-10 w-10 mx-auto mb-4 text-muted-foreground" />
            <p className="text-sm font-medium">
              {isDragActive ? 'Drop the CSV file here...' : `Drag and drop a ${importType === 'site-data' ? 'site data' : 'CRA list'} CSV file here, or click to select`}
            </p>
            ...
            {parseError ? <div className="bg-red-50 p-4 rounded-md flex items-start space-x-2">
                <AlertCircle className="h-5 w-5 text-red-500 mt-0.5" />
                <div>
                  <p className="font-medium text-red-800">Error parsing CSV</p>
                  <p className="text-sm text-red-700">{parseError}</p>
                </div>
              </div> : importType === 'site-data' && parsedSiteData.length > 0 || importType === 'cra-list' && parsedCRAData.length > 0 ? <div className="space-y-4">
                ...
                {stats && <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-md">
      ...
      <CardFooter className="flex justify-end gap-2">
        <Button variant="outline" onClick={handleReset} disabled={!file || processing}>
          Cancel
        </Button>
        <Button onClick={handleUpload} disabled={importType === 'site-data' && !parsedSiteData.length || importType === 'cra-list' && !parsedCRAData.length || processing || !projectId} className="bg-blue-500 hover:bg-blue-600 text-white">
